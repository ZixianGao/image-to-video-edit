...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, freqs, expert_token=None):
    ...
    # def forward(self, x, freqs, expert_token=None):
    #     """
    #     x: (B, N, D)           -> backbone tokens
    #     expert_token: (B, M, D) -> optional expert tokens, 只影响 attention 分数
    #     """
    #     q = self.norm_q(self.q(x))
    #     k = self.norm_k(self.k(x))
    #     v = self.v(x)
    ...
    #         k_full = torch.cat([k, k_expert], dim=1)  # (B, N+M, D)

    #         v_expert = torch.zeros_like(k_expert)
    #         v_full = torch.cat([v, v_expert], dim=1)

    #         x_full = self.attn(q, k_full, v_full)
...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, context, t_mod, freqs,expert_token=None):
        ...
        x = self.gate(x, gate_msa, self.self_attn(input_x, freqs,expert_token=expert_token))
